...
export const MsgVerify = {
  encode(message: MsgVerify, writer: Writer = Writer.create()): Writer {
    if (message.creator !== "") {
      writer.uint32(10).string(message.creator);
    }
    if (message.alias !== "") {
  ...
  decode(input: Reader | Uint8Array, length?: number): MsgVerify {
    const reader = input instanceof Uint8Array ? new Reader(input) : input;
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = { ...baseMsgVerify } as MsgVerify;
    while (reader.pos < end) {
      const tag = reader.uint32();
      switch (tag >>> 3) {
        case 1:
...
const baseMsgVerifyResponse: object = {};

export const MsgVerifyResponse = {
  encode(_: MsgVerifyResponse, writer: Writer = Writer.create()): Writer {
    return writer;
  },
  ...
  decode(input: Reader | Uint8Array, length?: number): MsgVerifyResponse {
    const reader = input instanceof Uint8Array ? new Reader(input) : input;
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = { ...baseMsgVerifyResponse } as MsgVerifyResponse;
    while (reader.pos < end) {
      const tag = reader.uint32();
      switch (tag >>> 3) {
        default:
...
export class MsgClientImpl implements Msg {
  ...
  Verify(request: MsgVerify): Promise<MsgVerifyResponse> {
    ...
    return promise.then((data) => MsgVerifyResponse.decode(new Reader(data)));
  }
}
